...
void layernorm_sw(int16_t* act, int16_t* y, int16_t* norm_weight, int16_t* norm_bias, float scaling_factor)
{
    ...
    // perform el-wise mac
    for (int i = 0; i < CFG::dmodel * CFG::seqlen; ++i){
        y[i] = int16_t((y[i] * norm_weight[i] + norm_bias[i])*scaling_factor);
    }
...
{
    int32_t* fc_out = new int32_t[CFG::seqlen * CFG::ffdim];
    int8_t* fc_out_quant = new int8_t[CFG::seqlen * CFG::ffdim];
    int16_t* ln_fc_in = new int16_t[CFG::seqlen * CFG::dmodel];
    int16_t* ln_out = new int16_t[CFG::seqlen * CFG::dmodel];

    linear_sw(fc_in, dense_weight_t, dense_bias, fc_out, CFG::seqlen, CFG::ffdim, CFG::dmodel);
